...
Once you've made your changes to the MedRecords codebase and are ready to share them, you'll submit a pull request (PR). Here's a guide to walk you through the process:
...
- MedRecords requires 100% Python isolated test coverage. That means that each module should be tested on its own (isolated coverage).
- For every Python file in the main `medrecords/` directory, there has to be a matching test file in the `tests/` directory (e.g., `medrecords/utils.py` → `tests/test_utils.py`).
...
- Navigate to the [MedRecords pull requests](https://github.com/medmodels/medrecords/pulls) and create a new PR.
