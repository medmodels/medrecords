...
class TestAnalyticEntry(unittest.TestCase):
    ...
    def test_value(self) -> None:
        assert math.isclose(create_analytic_entry(result=1).value(float), 1.0)

        table = Table(["amet"], [[1]])
        measurement = Measurement(1.0, lower=0.5, upper=1.5)
        assessment = Assessment(3, "at least 5", passed=False)
        distribution = Distribution([1.0, 2.0])

        assert create_analytic_entry(result=1).value(int) == 1
        assert math.isclose(create_analytic_entry(result=1.5).value(float), 1.5)
        assert create_analytic_entry(result="sit").value(str) == "sit"
        assert create_analytic_entry(result=True).value(bool) is True
        assert create_analytic_entry(result=table).value(Table) == table
        assert create_analytic_entry(result=create_plot()).value(Plot) == create_plot()
        assert (
            create_analytic_entry(result=measurement).value(Measurement) == measurement
        )
        assert create_analytic_entry(result=assessment).value(Assessment) == assessment
        assert (
            create_analytic_entry(result=distribution).value(Distribution)
            == distribution
        )

    def test_invalid_value(self) -> None:
        with pytest.raises(TypeError, match="it holds Failure"):
            create_analytic_entry(result=create_failure()).value(int)

        with pytest.raises(TypeError, match="cannot read 'amet' as int: it holds True"):
            create_analytic_entry(result=True).value(int)
...
class TestReport(unittest.TestCase):
    def test_init(self) -> None:
        ...
        assert report.analytic("amet").value(int) == 1
        ...
        assert (
            report.group("sit").group("elit").analytic("consectetur").value(str)
            == "dolor"
        )
