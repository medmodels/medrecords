...
class ByNode(Derivation[MedRecord, Mapping[str, MedRecord]]):
    def derive(self, medrecord: MedRecord) -> Dict[str, MedRecord]:
        return {name: medrecord.keep_nodes([name]) for name in NODE_NAMES}


class RaisingKeys(Derivation[MedRecord, Mapping[str, MedRecord]]):
    def derive(self, medrecord: MedRecord) -> Dict[str, MedRecord]:
        msg = "labore"
        raise ValueError(msg)


class NoKeys(Derivation[MedRecord, Mapping[str, MedRecord]]):
    def derive(self, medrecord: MedRecord) -> Dict[str, MedRecord]:
        return {}
...
class TestRun(unittest.TestCase):
    def test_report(self) -> None:
        ...
        assert report.analytic("count").value(int) == 3
        ...
        assert first.analytic("count").value(int) == 1
        assert first.analytic("edges").value(int) == 0
        assert first.group("nested").inputs == [InputEntry("medrecord", 1, 0)]
        assert first.group("nested").analytic("count").value(int) == 1
        ...
        assert keyed.group("ipsum").analytic("count").value(int) == 1
        ...
        assert progress[0]["entry"].value(int) == 3
    ...
    def test_invalid_report(self) -> None:
        ...
        assert create_report(strict).analytic("counting").value(int) == 3
