...
MEDRECORDS_ROOT = PROJECT_ROOT / "medrecords"
...
def map_to_test(module_path: Path) -> Path:
    ...
    Returns:
        Path: full test path
    """
    rel = module_path.relative_to(MEDRECORDS_ROOT)
...
def main() -> None:
    ...
    modules = {
        ...
        for module_path in MEDRECORDS_ROOT.rglob("*.py")
    ...
    for module_path, (test_path, coverage_path) in modules.items():
        if test_path.exists():
            logger.info(
                ...
                test_path,
                module_path.relative_to(MEDRECORDS_ROOT),
            )
            subprocess.run(
                [
                    sys.executable,
