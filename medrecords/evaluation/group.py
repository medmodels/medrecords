...
class _GroupOverMember(_Member[InputsType], Generic[InputsType, DerivedType]):
    ...
    def _entry(self, run: Run, inputs: InputsType, path: EntryPath) -> GroupEntry:
        ...
        A derivation that fails, in ``derive`` or in ``summarize``, leaves the
        group with the enclosing inputs and without entries, and its failure on
...
class _GroupOverEachMember(_Member[InputsType], Generic[InputsType, DerivedType]):
    ...
    _derivation: Derivation[InputsType, Mapping[str, DerivedType]]
    _group: EvaluationGroup[DerivedType]
    _title: str
    _description: Optional[str]
    ...
    def __init__(
        self,
        name: str,
        derivation: Derivation[InputsType, Mapping[str, DerivedType]],
        group: EvaluationGroup[DerivedType],
        title: Optional[str],
        description: Optional[str],
    ) -> None:
        ...
        Args:
            ...
            derivation (Derivation[InputsType, Mapping[str, DerivedType]]): The
    ...
    def _entry(self, run: Run, inputs: InputsType, path: EntryPath) -> GroupEntry:
        ...
        A derivation that fails, in ``derive`` or in ``summarize``, leaves the
        group without entries and its failure on the derivation entry.
        ...
        entries: List[Entry] = [
            GroupEntry(
                ...
                None,
                self._group._entries(run, inner, (*child_path, key)),
            )
...
class EvaluationGroup(Generic[InputsType]):
    ...
    Every ``add_*`` returns a new group; names are unique within a group and
    become the path of the entry in the report. A group is never mutated, so one
    group can sit in several evaluations and several runs at once.
    """
