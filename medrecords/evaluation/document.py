...
class Document:
    ...
    A template receives the report as ``json(bytes(sys.inputs.report))``, the
    logo as SVG text in ``sys.inputs.logo``, empty for a document without one,
    and the settings as ``json(bytes(sys.inputs.options))``: ``kind``,
    ``run_details``, ``section_breaks`` and ``company``. The bundled template
    honours all of them; a custom template reads what it needs.

    The bundled template prints the analytics of a group before its groups,
    whatever order they were added in, so that an analytic of the outer group
    does not end up under the heading of an inner one.
    """
    ...
    def _logo_text(self) -> str:
        """Reads the logo, which a template cannot open itself.

        Typst resolves paths against the directory of the template it compiles,
        so a file anywhere else has to be handed over as text.
