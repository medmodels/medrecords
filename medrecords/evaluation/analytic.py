...
class Figure(Protocol):
    ...
    Matplotlib figures and seaborn grids satisfy it.
    """
...
class Assessment:
    ...
    The check itself is the analytic's job, in the query engine or in Python.
    The assessment records what was found, what was required of it and whether
...
class Distribution:
    ...
    def std(self) -> float:
        ...
        The query engine measures the same way, so a distribution and an
        analytic reading the attribute directly report the same number.
    ...
    def histogram(self, bins: Optional[int] = None) -> List[Bin]:
        """Counts the values in bins of equal, rounded width covering every value.

        The bin width is 1, 2 or 5 times a power of ten, the smallest such width
        that covers the values with the asked number of bins, and the bin edges
        are multiples of that width, so the edges read naturally. Whole numbers
        are never split into fractions of a bin. Rounding the width can leave
        the histogram with fewer bins than asked for, or with one more. When the
        values are the same, or so close together that a bin edge cannot move
        between them, the histogram holds one bin.
...
class Analytic(ABC, Generic[InputsType]):
    """Base class for analytics: one computation over inputs, reporting one value.

    ``compute`` is the only method a subclass must define. A run calls it once
    wherever it was added, and once per key under a mapping derivation, times it
    and records what it returned or raised.
    The inputs are passed positionally, so a subclass names the parameter after
    what it receives (``medrecord`` for a MedRecord). A ``QueryError`` the query engine
    hands back in band is recorded as a failure, exactly like a raised exception.
    ``title`` and ``description`` shape how the entry is typeset; ``parameters``
    records what the analytic was configured with. One instance can be placed in
    several groups and evaluations and is computed once for each of them, so
    ``compute`` must not mutate it: what it wrote would leak into the next run.
    """
...
class Derivation(ABC, Generic[InputsType, DerivedType]):
    """Base class for derivations: shared work computed once per group.

    ``derive`` turns the inputs of a group into the inputs of a nested group: a
    cohort, precomputed statistics, a fitted model. Placed with
    ``add_group_over_each`` it returns one inputs binding per key, and the nested
    group runs once per key. ``parameters`` records what the derivation was
    configured with and ``summarize`` what it produced. One instance can be
    added to several groups and derives once for each of them, so ``derive``
    must not mutate it: what it wrote would leak into the next run.
    """
    ...
    def summarize(self, derived: DerivedType, /) -> Mapping[str, Detail]:
        ...
        The sizes of derived MedRecords are recorded by the run itself; this is
        for what a run cannot see, such as a fitted model or an index. A summary
        that fails fails the derivation, so the nested group does not run.
